...
namespace ScheduleIMod.Tests.Logic
{
    public class CooldownLogicTests
    {
        ...
        [Fact]
        public void ShouldAllowDeal_ReturnsCorrectRemainingSeconds()
        {
            var result = CooldownLogic.ShouldAllowDeal(
                timeSinceLastCompleted: 100f,
                timeSinceLastOffered: 1000f,
                hasPendingOffer: false,
                hasPendingInstantDeal: false,
                cooldownMultiplier: 1.0f, // 360s cooldown
                out string invalidReason,
                out float remainingSeconds
            );

            result.Should().BeFalse();
            remainingSeconds.Should().Be(260f); // 360 - 100 = 260
            invalidReason.Should().Contain("recently completed");
        }
    }
}
