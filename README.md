...
### Cooldown Tracker UI (IL2CPP Only)
...
- **Toggle Visibility**: Press `[` (default) to show/hide the tracker window
- **Auto-Cleanup**: Expired cooldowns automatically remove from the list
...
- `CooldownWindowX/Y`: Window position (automatically saved when dragging)
- `ShowCooldownWindow`: Window visibility state
- `ToggleWindowKey`: Keybind to toggle window (default: `[` left bracket)
